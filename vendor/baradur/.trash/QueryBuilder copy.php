...
Class QueryBuilder
{
    ...
    {
        $res = $this->_method . ' `' . $this->_table . '` ' . $this->_join . ' ' 
                . $this->_where . $this->_union . $this->_order . ' ' . $this->_limit;
   ...
   /**
    ...
    public function get()
    {
        ...
        $sql = $this->_method . ' `' . $this->_table . '` ' . $this->_join . ' ' . $this->_where 
                . $this->_union . $this->_order . ' ' . $this->_limit;
    ...
    public function paginate($cant = 10)
    {
        ...
        $sql = $this->_method . ' `' . $this->_table . '` ' . $this->_join . ' ' . $this->_where 
                . $this->_union . $this->_order . ' LIMIT ' . $offset . ', ' . $cant;
        ...
        $records = 'SELECT COUNT(*) AS total FROM `' . $this->_table . '` ' . $this->_where;
        ...
        {
            $pagination = new arrayObject();
            $pagination->first = $pagina<=1? null : http_build_query($otros,'','&').'&pagina=1';
            $pagination->second = $pagina<=1? null : http_build_query($otros,'','&').'&pagina='.($pagina-1);
            $pagination->third = $pagina==$pages? null : http_build_query($otros,'','&').'&pagina='.($pagina+1);
            $pagination->fourth = $pagina==$pages? null : http_build_query($otros,'','&').'&pagina='.$pages;
    ...
    public function processRelationship($class, $foreign, $primary, $relationship)
    {
        ...
        $wherein = $this->recusiveSearch($this->_collection, $primary, $this->_rparent, $wherein);

        //echo "WHEREIN: ".implode(',',$wherein)."<br><br>";
        return $res->whereIn($foreign, implode(',',$wherein));
    ...
    {
        //echo "RELATIONSHIP THROUGH : ".$this->_rparent."<br>";
        ...
        $key = $this->_rparent? $primarythrough : $primary;
        $keythrough = $this->_rparent? $foreign : $foreignthrough;

        $wherein = $this->recusiveSearch($this->_collection, $key, $this->_rparent, $wherein);

        //echo "WHEREIN: ".implode(',',$wherein)."<br><br>";
        return $res->whereIn($keythrough, implode(',',$wherein));
        //echo $res->toSql() ."<br>";
        //return $res;
    }
    ...
    private function addWith($function, $parent=null, $extrawhere)
    {
        #echo "ADDING WITH: ".$function."<br>";
        ...
        if (strpos($function, ':')>0) {
            list($function, $columns) = explode(':', $function);
            $columns = explode(',', $columns);
        }
        
        $extra = new $this->_parent;
        $extra->getQuery()->_collection = $this->_collection;
        $extra->getQuery()->_rparent = $parent;
        if (!$this->_original)
            $extra->getQuery()->_original = Helpers::getTableNameFromClass($this->_parent, false);
       ...
       // dd($this_collection);
        ...
        if (isset($this->_eagerLoad))
        {
            ...
            {
                //echo "EXTRAQUERY:::::";var_dump($relation['relation']);echo"<br>";
                list($rel_parent, $rel_son) = explode('.', $relation['relation']);

                if (isset($relation['constraints']))
                {
                    $exw = $relation['constraints'];
                    //echo "::::EX::::::".($exw->_relation).":::"; var_dump($exw); echo "<br>";
                    if ($exw->_relation == $function)
                    {
                        $extra->whereRaw(str_replace('WHERE ', '', $exw->_where));
                    }
                    elseif (isset($rel_son) && $rel_parent == $function)
                    {
    ...
    public function _has($relation, $extraquery=null, $comparator=null, $value=null)
    {
        ...
        $filter = '';
        if (isset($extraquery) && !is_array($extraquery))
        {
            $filter = str_replace('WHERE', ' AND', $extraquery->_where);
        } 
        elseif (isset($extraquery) && is_array($extraquery))
        {
            foreach ($extraquery as $exq)
            {
    ...
    public function _withWhereHas($function, $filters=null)
    {
        ...
        $this->_extraquery = array($function => $filters);

        array_push($this->_extra, array('function' => trim($function), 'where' => $filters));

        return $this->_has($function); */
    ...
    {
        //echo "<br>SCOPE: scope".ucfirst($scope)."<br>";
        $func = 'scope'.ucfirst($scope);

        return call_user_func_array(array($this->_parent, $func), array_merge(array($this), $args));
