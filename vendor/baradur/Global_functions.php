...
function app($val = null)
{
	global $app;
	return $app->instance($val);
}
function asset($val)
{
	return View::getAsset($val);
}
function route()
{
	return Route::getRoute(func_get_args());
}
function to_route()
{
	return redirect(Route::getRoute(func_get_args()));
}
function __($translation, $placeholder = null)
{
	return Helpers::trans($translation, $placeholder);
}
...
{
	return _DIR_ . 'resources/' . $path;
}
function storage_path($path = null)
{
	return _DIR_ . 'storage/' . $path;
}
function database_path($path = null)
{
	return _DIR_ . 'database/' . $path;
}
...
function csrf_token()
{
	return App::getRequestToken();
}
function config($val, $default = null)
{
	return Helpers::config($val, $default);
}
function class_basename($name)
{
...
{
	return $path ? app('UrlGenerator')->to($path, $parameters, $secure) : app('UrlGenerator');
}
 ...
 * @return Response
 */
function redirect($url = null, $code = 200)
{
	$old = array();
	...
	//$_SESSION['old'] = $old;
	session()->flash('_old', $old);
	session()->flash('messages', session('messages'));
...
function __dump($content, $full = false, $die = false)
{
	...
	foreach ($trace as $tr) {
		if (!Str::contains($tr['file'], DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'baradur')) {
			$called_from = $tr;
			break;
		}
	}
...
{
	return _DIR_ . 'app' . DIRECTORY_SEPARATOR . $value;
}
...
{
	return Prompts::show_note($message, 'outro');
}
function intro($message)
{
	return Prompts::show_note($message, 'intro');
}
function note($message)
{
	return Prompts::show_note($message, 'note');
}
function info($message)
{
	return Prompts::show_note($message, 'info');
}
function warning($message)
{
	return Prompts::show_note($message, 'warning');
}
function error($message)
{
	return Prompts::show_note($message, 'error');
}
function alert($message)
{
	return Prompts::show_note($message, 'alert');
}
