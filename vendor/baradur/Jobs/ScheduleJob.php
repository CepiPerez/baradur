...
Class ScheduleJob
{
    ...
    public function filtersPass()
    {
        ...
        foreach ($this->filters as $callback) {
            list($c, $m, $p) = getCallbackFromString($callback);
            
            if (! $this->__executeCallback($m, $p)) {
                return false;
            }
        }
    ...
    {
        $this->filters[] = is_closure($callback) 
            ? $callback 
            : array("ScheduleJob|__defaultClosure|1", $callback);
    ...
    {
        $this->rejects[] = is_closure($callback) 
            ? $callback 
            : array("ScheduleJob|__defaultClosure|1", $callback);
    ...
    public function __executeCallback($method, $params)
    {
    ...
    public function __betweenClosure($now, $startTime, $endTime)
    {
        //dump("BETWEEN", $now, $startTime, $endTime, $now->between($startTime, $endTime));
        return $now->between($startTime, $endTime);
    }

    public function __defaultClosure($value)
    {
        return $value;
    }
    ...
    private function inTimeInterval($startTime, $endTime)
    {
        ...
        return array("ScheduleJob|__betweenClosure|3", $now, $startTime, $endTime);
